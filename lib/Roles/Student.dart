...
Future<String> getUserName(String username) async {
  ...
  if (user != null) {
    ...
    await user.getIdToken(); // Refresh the user's ID token

    return user.displayName ?? ''; // Return the user's name or an empty string if not available
  }
...
class _StudentState extends State<Student> {
  ...
  var rooll;
  var emaill;
  String userName = ''; // Add this variable to store the user's name
  ...
  void initState() {
    ...
    // Retrieve the user's name and update the 'userName' variable
    ...
    return Scaffold(
      body: StreamBuilder(
        ...
        builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
          ...
          return ShowData();
          // return HomeScreen(id: id, email: emaill, username: userName);
          // return Container(
          //   decoration: BoxDecoration(
          //     borderRadius: BorderRadius.circular(12),
          //   ),
